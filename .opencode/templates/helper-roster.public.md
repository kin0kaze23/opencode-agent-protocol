...
> YOUR_PROVIDER/YOUR_PROVIDER decommissioned from workspace routing (quota exhausted, 429 insufficient_quota).
> Canonical candidate registry: `.opencode/model-registry.yaml`. Secrets are sourced from
> YOUR_SECRET_MANAGER project `YOUR_PROJECT`, config `dev_backend`, by variable name only.

Use this roster when the Owner decides to delegate or switch reasoning depth.

## Routing source of truth

- `.opencode/opencode.json` sets the **Owner session default** model: `YOUR_PROVIDER/umans-coder` (v1.5 capacity-first).
- Direct bounded implementation helper: `YOUR_PROVIDER/umans-coder` (v1.5 capacity-first).
- Fallback/hard-solver/rollback: `YOUR_PROVIDER/qwen3.7-plus` (premium reserve).
- This helper roster sets **delegated helper defaults**: `YOUR_PROVIDER/umans-coder` for Orchestrator/Planner/Implementer (capacity-first), `YOUR_PROVIDER/glm-5.1` for Reviewer (premium reserve), `YOUR_PROVIDER/qwen3.7-plus` for Architect (premium reserve), and `YOUR_PROVIDER/deepseek-v4-flash` for Explorer/Budget.
...
- OpenCode Go routing is promoted for OpenCode runtime agents only. Do not remove YOUR_PROVIDER/YOUR_PROVIDER fallback, migrate Hermes/direct API, or migrate Claude wrappers as part of Phase M1.3.
- `YOUR_PROVIDER/umans-coder` is the v1.5 capacity-first Owner/Planner/Implementer route; `YOUR_PROVIDER/qwen3.7-plus` is the premium reserve for Architect and high-risk tasks; ambiguous performance requests remain plan-first under protocol guardrails.
- `YOUR_PROVIDER/qwen3.6-plus` is the fallback/hard-solver/rollback baseline.
...
- **v0.2 eval (2026-06-05):** `YOUR_PROVIDER/qwen3.6-plus` confirmed as production baseline — only model with 0 empty responses across all roles.
- **Action 4B (2026-06-05):** `YOUR_PROVIDER/qwen3.7-plus` scored 10.0/10, 100% pass, 0 empty — approved for canary (Action 4C).
- **Action 4D (2026-06-05):** `YOUR_PROVIDER/qwen3.7-plus` promoted to production v1.1 after 3 clean canary tasks.
- **v0.2 eval (2026-06-05):** `YOUR_PROVIDER/mimo-v2.5-pro` blocked from automatic routing — produced 2 empty responses on debugging and feature implementation tasks.
- **v0.2 eval (2026-06-05):** `YOUR_PROVIDER/deepseek-v4-pro` blocked from hard-solver/debugging — produced 2 empty responses on debugging and build recovery tasks.
- **v0.2 eval (2026-06-05):** `YOUR_PROVIDER/minimax-m2.7` blocked from implementation — implementer score 2.59/10, too weak for production.
...
|---|---|---|---|---|
| Orchestrator | `YOUR_PROVIDER/umans-glm-5.2` | ✅ production_active_for_orchestrator_routine_medium | Routine/medium-risk orchestration, task coordination. Fallback: umans-coder → YOUR_PROVIDER/qwen3.7-plus (premium reserve). | High-risk architecture, difficult debugging (use OpenCode Go) |
| Planner | `YOUR_PROVIDER/umans-coder` | ✅ production_active_soak_monitoring | Feature plans, plan correction, implementation-readiness checks | Writing production code |
| Implementer | `YOUR_PROVIDER/umans-coder` | ✅ production_active_soak_monitoring | Approved code changes on explicit coding specs with reviewer gate. Fallback: qwen3.7-plus. | Ambiguous tasks, autonomous execution, guardrail decisions |
| Reviewer | `YOUR_PROVIDER/umans-glm-5.1` | ✅ production_active_for_reviewer_judge | Repo-grounded review, safety second-pass, risk verdicts. Fallback: YOUR_PROVIDER/glm-5.1 (premium reserve). | Default executor, autonomous planning, bulk use |
| Explorer | `YOUR_PROVIDER/umans-flash` | ✅ production_active_for_explorer_budget | Fast repo mapping and runtime-path discovery. Fallback: YOUR_PROVIDER/deepseek-v4-flash. | Final correctness verdicts |
| Architect | `YOUR_PROVIDER/qwen3.7-plus` | ✅ v1.1-production | High-risk auth, schema, state-model decisions (premium reserve) | Routine implementation or bulk review |
| Budget | `YOUR_PROVIDER/umans-flash` | ✅ production_active_for_explorer_budget | Read-only cost/routing summaries, cheap classification. Fallback: YOUR_PROVIDER/deepseek-v4-flash. | Editing files |
| Judge (eval) | `YOUR_PROVIDER/umans-glm-5.1` | ✅ production_active_for_reviewer_judge | Sparse high-value review/judge checks. Fallback: YOUR_PROVIDER/glm-5.1. | Broad repeat matrices |

## Routing logic (v4.8.0 — historical reference)

v4.8.0 introduced **YOUR_PROVIDER-first model routing** based on 6 evidence phases (Phase 3–6B). This section is historical reference only and has been superseded by v1.5 capacity-first routing.
...
> **Note (v1.1-production, 2026-06-12):** All routes in the v4.8.0 table above have been superseded by `qwen3.7-plus` (production primary) and `qwen3.6-plus` (fallback). The following models are historical/eval-only references and are NOT in active production routing: `qwen3.5-plus` (decommissioned — YOUR_PROVIDER quota exhausted), `qwen3-coder-plus` (rejected — 3.73 avg, 1 hard-fail), `qwen3-coder-next` (rejected — 43.3 guardrail score, 6 blockers), `qwen3-max-2026-01-23` (not available on OpenCode Go). The ModelEval helper rows below are for evaluation mode only and must not be confused with production routing.

## Recommended switch guidance

- Start on `YOUR_PROVIDER/umans-coder` for Owner, planning, and implementation (v1.5 capacity-first).
- Switch to `YOUR_PROVIDER/qwen3.7-plus` for high-risk architecture, difficult debugging, final review, or when Umans fails quality/tool-use checks.
- Use `YOUR_PROVIDER/qwen3.6-plus` for hard-solver/fallback/rollback tasks. Switch when the plan is explicit and rollback is needed.
- Use `YOUR_PROVIDER/glm-5.1` for sparse `/review` and judge work only; avoid bulk use.
- Use `YOUR_PROVIDER/deepseek-v4-flash` for Explorer/Budget cheap read-only work.
- Keep `YOUR_PROVIDER/kimi-k2.6` as manual high-risk senior reviewer (eval-passed Phase 3B), `YOUR_PROVIDER/deepseek-v4-pro` as hard deep-repo solver only, and `YOUR_PROVIDER/minimax-m3` as manual UI/multimodal QA specialist only when OpenCode Go quota is available (v4.15.2: automatic visual-reviewer routing moved to `YOUR_PROVIDER/umans-kimi-k2.7` because minimax-m3 returned Insufficient balance).

## Visual Reviewer Fallback Diversity Plan (v4.16)

As of v4.16, `visual-reviewer-fallback` routes to `YOUR_PROVIDER/kimi-k2.6` (OpenCode Go premium reserve), providing true provider+model diversity from the primary `YOUR_PROVIDER/umans-kimi-k2.7` (Umans).

**v4.16 canary routing:**
- Primary: `YOUR_PROVIDER/umans-kimi-k2.7` (Umans) — routine visual QA
- Fallback: `YOUR_PROVIDER/kimi-k2.6` (OpenCode Go) — second opinion, primary failure, or high-risk visual QA
...
|---|---|---|---|---|
| **Orchestrator** | `YOUR_PROVIDER/umans-glm-5.2` | `YOUR_PROVIDER/umans-coder` | `YOUR_PROVIDER/qwen3.7-plus` | v1.5.2: production_active_for_orchestrator_routine_medium |
| **Implementer** | `YOUR_PROVIDER/umans-coder` | `YOUR_PROVIDER/qwen3.7-plus` | `YOUR_PROVIDER/qwen3.6-plus` | Capacity lane |
| **Reviewer** | `YOUR_PROVIDER/umans-glm-5.1` | `YOUR_PROVIDER/glm-5.1` | — | Umans primary (v1.5.2). OpenCode Go premium reserve for high-risk. |
| **Planner** | `YOUR_PROVIDER/umans-coder` | `YOUR_PROVIDER/qwen3.7-plus` | `YOUR_PROVIDER/qwen3.6-plus` | Capacity lane |
| **Architect** | `YOUR_PROVIDER/qwen3.7-plus` | `YOUR_PROVIDER/qwen3.6-plus` | `YOUR_PROVIDER/umans-glm-5.2` | Premium reserve for high-risk architecture |
| **Explorer** | `YOUR_PROVIDER/umans-flash` | `YOUR_PROVIDER/deepseek-v4-flash` | — | v1.5.2: production_active_for_explorer_budget |
| **Budget** | `YOUR_PROVIDER/umans-flash` | `YOUR_PROVIDER/deepseek-v4-flash` | — | v1.5.2: production_active_for_explorer_budget |
| **Compaction** | `YOUR_PROVIDER/glm-5.2` | `YOUR_PROVIDER/umans-kimi-k2.7` (<=180K only) | `YOUR_PROVIDER/qwen3.7-plus` (<=100K only) | GLM-5.2 has ~1M context, matches chat model. Kimi bounded to <=180K. |
| **Summary** | `YOUR_PROVIDER/glm-5.2` | `YOUR_PROVIDER/umans-kimi-k2.7` (<=180K only) | `YOUR_PROVIDER/qwen3.7-plus` (<=100K only) | GLM-5.2 has ~1M context. Kimi bounded to <=180K. |
...
All API keys are managed through YOUR_SECRET_MANAGER (project: `YOUR_PROJECT`, config: `dev_backend`). Agent may verify secret names/existence only. Never print, log, commit, or document secret values.

## Token and model-efficiency rules (M1.7)

- **Cheap-first read-only work:** use Explorer or Budget on `YOUR_PROVIDER/deepseek-v4-flash` for routine repo lookup, quota/cost checks, and low-risk classification before spending umans-coder or qwen3.7-plus context.
- **umans-coder is capacity-first primary:** use for routine orchestration, planning, and implementation. Reserve `YOUR_PROVIDER/qwen3.7-plus` for high-risk architecture, difficult debugging, final review.
...
- **Implementer consumes plans:** Implementer on `YOUR_PROVIDER/umans-coder` executes the approved touch list and must not redo planning, expand scope, or make architecture decisions.
- **Reviewer cost guard:** use `YOUR_PROVIDER/umans-glm-5.1` for routine review. Escalate to `YOUR_PROVIDER/glm-5.1` (premium reserve) only for risk score 4+, sensitive paths, auth/security/payment/data/secrets changes, 4+ changed files, release/ship gates, unclear implementation quality, or explicit owner request. For low-risk DIRECT/FAST work, sample review instead of reviewing every change.
...
- YOUR_PROVIDER models disagree materially and a tie-breaker is required.
...
|---|---|---|---|---|
| `YOUR_PROVIDER/kimi-k2.6` | Senior reviewer | Phase 3B: 6/6 PASS, 4.8/5 | High-risk review, security/auth/schema, release readiness, protocol routing | Default reviewer, automatic delegation, implementation, protocol seal |
| `YOUR_PROVIDER/minimax-m3` | UI/multimodal QA | Phase 3C: 5/5 PASS, 4.6/5 | Screenshot QA, mobile responsive, accessibility audit, theme review, product handoff | Security review, implementation, protocol seal, production deploy, automatic routing |
...
These helpers are for protocol model comparison evaluation only. They map all 9 YOUR_PROVIDER Model Studio coding plan models to standardized evaluation roles.
...
**Evaluation protocol:** Each model receives the same standardized task prompt. Outputs are scored against the shared eval rubric in `docs/evals/framework/SCORING_GUIDE.md` (v2.0, 8-category). Results are written to `docs/evals/models/results/`.
