...
Public drift hardening + sync guardrails. Synced Visual QA protocol from internal repo to visual-reviewer agent definitions and prompt mirrors — replaced `READY TO SHIP`/`NEEDS FIXES` verdicts with `TECHNICAL_VISUAL_PASS`/`TECHNICAL_VISUAL_FAIL` and added verdict scope note. Added `scripts/validate-public-sync.sh` — drift detection script that checks version consistency, forbidden author-specific strings, visual reviewer verdicts, stale v4 banners, placeholder model IDs, vault/evals references, and prompt mirror existence. Sanitized `.opencode/AGENTS.md`, `.opencode/rules.md`, and `.opencode/helper-roster.md` of all author-specific content — replaced model IDs with `YOUR_PROVIDER/YOUR_*_MODEL` placeholders, removed `Doppler`/`nuggie-be`/`Alibaba`/`Bailian`/`example-agent` references, removed `vault/evals/` references, and added Visual QA & Design Review Protocol section. Added `public-sync-validation` job to CI (Ubuntu + macOS). Created `docs/PUBLIC_SYNC_MANIFEST.md` documenting sanitized files and forbidden patterns. Updated version files to v5.5.4.
